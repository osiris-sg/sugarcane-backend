...
MODEL_PATH = join(dir, '..', 'ml', 'sales_prediction_model.joblib')


def prepare_features_and_predict(df_raw, predict_date=None):
    """
    ...
    # For lag_1, use the last day's sales
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            if not historical_data:
                ...
                self.end_headers()
                self.wfile.write(json.dumps({'error': 'No historical data provided', 'success': False}).encode('utf-8'))
                return
            ...
            result = prepare_features_and_predict(df, predict_date)
    ...
    def do_GET(self):
        ...
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        self.wfile.write(json.dumps({'status': 'ok', 'message': 'Use POST to run prediction'}).encode('utf-8'))

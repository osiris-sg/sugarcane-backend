...
// Device ID swap mapping - swap these IDs when they come in from machines
const DEVICE_ID_SWAP_MAP = {
  '852356': '852346',
  '852346': '852356',
};

// Helper: Swap device IDs if they're in the swap map
export function swapDeviceId(deviceId) {
  const id = String(deviceId);
  return DEVICE_ID_SWAP_MAP[id] || id;
}

// Helper: Look up device name from Device table
// Returns the location field (used as display name), or falls back to deviceName, then provided name
export async function getDeviceNameById(deviceId, fallbackName = null) {
  try {
    const device = await db.device.findUnique({
      where: { deviceId: String(deviceId) },
      select: { deviceName: true, location: true },
    });

    // Prefer location field as the display name for notifications
    if (device?.location) {

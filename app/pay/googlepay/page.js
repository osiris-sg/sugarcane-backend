...
export default function GooglePayPage() {
  ...
  useEffect(() => {
    ...
    script.onload = async () => {
      try {
        ...
        // Redirect to Airwallex checkout with ONLY Google Pay
        payments.redirectToCheckout({
          intent_id: intentId,
          client_secret: clientSecret,
          currency: currency,
          country_code: 'SG',
          methods: ['googlepay'], // Only show Google Pay!
  ...
  return (
    <html>
      <head>
        <title>Google Pay - Sugarcane Juice</title>
      ...
      <body>
        <div className="container">
          ...
          <div className="title">Google Pay</div>
          {status === 'loading' && (
            <>
          ...
          {status === 'initializing' && (
            <>
              <div className="spinner"></div>
              <div className="status">Initializing Google Pay...</div>
            </>
          )}
          {status === 'redirecting' && (
            <>
              <div className="spinner"></div>
              <div className="status">Redirecting to Google Pay...</div>
            </>
          )}
          {status === 'error' && (

...
export default function CardPayPage() {
  ...
  useEffect(() => {
    ...
    script.onload = async () => {
      try {
        ...
        // Redirect to Airwallex checkout with ONLY Card payment (for Samsung Pay)
        payments.redirectToCheckout({
          intent_id: intentId,
          client_secret: clientSecret,
          currency: currency,
          country_code: 'SG',
          methods: ['card'], // Only show Card payment!
  ...
  return (
    <html>
      <head>
        <title>Card Payment - Sugarcane Juice</title>
      ...
      <body>
        <div className="container">
          ...
          {status === 'initializing' && (
            <>
              <div className="spinner"></div>
              <div className="status">Initializing payment...</div>
            </>
          )}
          {status === 'redirecting' && (
            <>
              <div className="spinner"></div>
              <div className="status">Redirecting to payment...</div>
            </>
          )}
          {status === 'error' && (
